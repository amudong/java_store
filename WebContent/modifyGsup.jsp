<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
<%@ page import="demo.*"%>
<%@ page import="day4.*"%>
<%@ page import="java.util.*" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>修改页面</title>
</head>
<link href="css/style1.css" rel='stylesheet' type='text/css' media="all" />

<%List<gsupDB> g = gsup.select(); %>
<body>
<div class="insert1">
<form action="gsupServlet" method="get">
<input name="switch" type="text" value="2" style="position:absolute;
right:-3000px;">

		
			<br/>编号id：&nbsp;&nbsp;<input name="id" type="text" value="<%=request.getParameter("id")%>"/><br/>
			商品编号：&nbsp;<input name="gid" type="text" value="<%=request.getParameter("gid")%>"/><br/>
			供应商编号：<input name="gname" type="text" value="<%=request.getParameter("gname")%>"/><br/>
			供应商名称：<input name="price" type="text" value="<%=request.getParameter("price")%>"/><br/>
<br/>

		
	<input type="image" src="./img/submit.png" nclick="javascript:document.forms['borm_2'].submit();" style="    position: absolute;
    left: 570px;
    bottom: 30px;"/>
</form>
</div>
</body>
</html>